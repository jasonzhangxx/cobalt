...
// copying, or LinearizeData() to gather into contiguous scratch memory.
...
class MEDIA_EXPORT OffsetSegmentedByteQueue {
 public:
  ...
  // Like SegmentedByteQueue::LinearizeData(), except `offset` is absolute.
  // Returns false without writing anything when the requested range is not
  // entirely buffered, which includes any `offset` before head().
