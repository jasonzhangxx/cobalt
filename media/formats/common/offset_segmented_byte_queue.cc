...
bool OffsetSegmentedByteQueue::Trim(int64_t max_offset) {
  ...
  if (max_offset > tail()) {
    // Drop everything we do have, but report that the caller asked for more
    // than was buffered.
    Pop(queue_.size());
    return false;
  }
...
bool OffsetSegmentedByteQueue::LinearizeData(base::span<uint8_t> dest,
                                             int64_t offset) const {
  if (!IsReadableOffset(offset)) {
    return false;
  }
  return queue_.LinearizeData(dest, OffsetFromHead(offset));
}
