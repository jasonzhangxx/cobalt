...
void SegmentedByteQueue::Push(base::span<const uint8_t> data,
  ...
  LOG(ERROR) << "JA: " << __func__ << " this=" << this
             << " pushed=" << data.size() << " total=" << total_bytes_
             << " segments=" << segments_.size();
}

void SegmentedByteQueue::Pop(size_t count) {
  CHECK_LE(count, total_bytes_);
  const size_t popped = count;
  total_bytes_ -= count;
  ...
  LOG(ERROR) << "JA: " << __func__ << " this=" << this << " popped=" << popped
             << " total=" << total_bytes_ << " segments=" << segments_.size()
             << " released=" << fully_consumed;
}
...
bool SegmentedByteQueue::LinearizeData(base::span<uint8_t> dest,
  ...
  LOG(ERROR) << "JA: " << __func__ << " this=" << this << " size=" << size
             << " offset=" << offset << " total=" << total_bytes_
             << " segments=" << segments_.size();
