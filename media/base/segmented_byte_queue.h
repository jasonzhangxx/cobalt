...
//   * LinearizeData() gathers a byte range into caller-supplied contiguous
//     memory, for code that cannot deal with several runs. It is the fallback
//     for the cases GetContiguousData() cannot serve.
//
...
class MEDIA_EXPORT SegmentedByteQueue {
 public:
  ...
  // long, and otherwise falls back to a slow path, normally LinearizeData()
  ...
  // does not invalidate it: segments point at the caller's memory, not into
  ...
  //
  // As with ByteQueue::Peek(), the returned spans are only valid until the
  // next Pop() or Reset(); Push() does not invalidate them. `*segments` is
  // caller-owned, so reusing the same vector across calls avoids reallocating
  // the run list.
  ...
  // Copies the `dest.size()` bytes starting at `offset` into `dest`, gathering
  // across segment boundaries as needed. Returns false without writing
  // anything when fewer than `dest.size()` bytes are available from `offset`.
  //
  // This is the fallback for code that requires contiguous memory; prefer
  // GetSegmentedData() where several runs can be consumed directly, since it
  // avoids the copy.
  [[nodiscard]] bool LinearizeData(base::span<uint8_t> dest,
                                   size_t offset = 0u) const;
 ...
 private:
  struct Segment {
    ...
    // order, and `data` points into memory kept alive by `release`, so `data`
    // must be declared last in order to be destroyed first. Otherwise it would
    // briefly reference released memory, which BackupRefPtr can flag.

    // Keeps this segment's memory alive for as long as the segment is queued.
    base::ScopedClosureRunner release;

    // The unread bytes of this segment, owned by whoever called Push(). The
    // front segment's span is trimmed in place as bytes are popped, so it
    // always describes exactly the unread portion and is never empty.
    base::raw_span<const uint8_t> data;
  };

  // Maps `offset`, which is relative to the front of the queue, onto
  // {index of the segment containing it, offset of that byte within the
  // segment}. `offset` must be strictly less than total_bytes_.
  std::pair<size_t, size_t> Locate(size_t offset) const;

  std::vector<Segment> segments_;

  // Unread bytes across all segments.
